...
/*3*/ SELECT departamento.nombre, COUNT(profesor.id_profesor) AS 'Num_Profesores' FROM departamento JOIN profesor ON departamento.id = profesor.id_departamento GROUP BY departamento.nombre ORDER
...
/*6*/ SELECT grado.nombre, COUNT(asignatura.id) AS 'Total_Asignaturas' FROM grado LEFT JOIN asignatura ON grado.id = asignatura.id_grado GROUP BY grado.nombre HAVING COUNT(asignatura.id) > 40 ORDER BY Total_Asignaturas DESC;
